...
def _detect_corner_events(acc_arr, timestamp_arr):
    '''
    ...
    for k in range(len(acc_arr_abs)):
        t = timestamp_arr[k]
        acc = acc_arr_abs[k]
...
def _generate_graph(exp_path, dataset_name, session_timestamp, dataset_event_types, timestamp_arr_meas, values_arr_meas, timestamp_arr_gt, events_arr_gt, values_label, detection_label, detected_event_timestamps, detected_event_types, thresh_plot):
    '''
    ...
    ax[0].axhline(y=thresh_plot, color='b')
    ax[0].set_title(dataset_name + ' ' + values_label + ', with detected events')
    ...
    ax[1].set_title(dataset_name + ' ' + values_label + ', with ground truth events')
...
def process_dataset(session_timestamp, dataset_event_types, dataset_name, exp_path):
    ...
    # detect events
    detected_event_timestamps, detected_event_types = _detect_corner_events(acc_arr=acc_y_arr, timestamp_arr=timestamp_arr_acc)
